...
mosquito_naive_model <-
  wnv_case_temp_hvi_df |> 
  filter(year %in% 2021:2024) |> 
  glm(wnv_mosquitoes ~ 1,
      family = poisson, 
      data = _)
...
# LRT to compare saturated model  

-2 * (logLik(pos_mosquito_model) - logLik(pos_mosquito_model_saturated)) # 2 df
pchisq(-2 * (as.numeric(logLik(pos_mosquito_model)) - as.numeric(logLik(pos_mosquito_model_saturated))), df = 2, lower.tail = FALSE) # p value = 0.1318
...
pos_human_model_saturated <-
  wnv_case_temp_hvi_df |> 
  glm(wnv_cases ~ year + borough + avg_hvi + avg_temp + wnv_mosquitoes, 
      family = poisson,
      data = _)
...
# remove wnv+ mosquitoes as predictor - likely bc we only have mosquito data from 2021-2024

# testing with only 2021-2024 data
# saturated_human_2021_2024 <-
#   wnv_case_temp_hvi_df |> 
#   filter(year %in% 2021:2024) |> 
#   glm(wnv_cases ~ year + borough + avg_hvi + avg_temp + wnv_mosquitoes, 
#       family = poisson,
...
# 
# saturated_human_2021_2024 |> 
#   broom::tidy() # positive mosquitos detected predictor still not significant 


pos_human_model_saturated <-
  wnv_case_temp_hvi_df |> 
  glm(wnv_cases ~ year + borough + avg_hvi + avg_temp, 
      family = poisson,
      data = _)
...
pchisq(pos_mosquito_model_saturated$deviance, df = pos_mosquito_model_saturated$df.residual, lower.tail = FALSE) # p = 0.2239, not a good fit
pchisq(pos_human_model$deviance, df = pos_human_model$df.residual, lower.tail = FALSE) # p = 5.04e-34, not a good fit

pchisq(pos_human_model_saturated$deviance, df = pos_human_model_saturated$df.residual, lower.tail = FALSE) # p = 3.72e-32, not a good fit
